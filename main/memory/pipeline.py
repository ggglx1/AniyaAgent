...
class StructuredMemoryPipeline:
    ...
    def process(self, message_ids: list[str], user_id: str = "local") -> list[str]:
        ...
        for raw in self.extractor.extract(records) + self.llm_extractor.extract(records):
            ...
            if resolution == "conflict":
                ...
                record = self.manager.add(
                    ...
                    origin=candidate["origin"], valid_until=candidate["valid_until"], metadata={"source_message_ids": candidate["source_message_ids"]}, reason="conversation memory pipeline",
                )
    ...
    def route_tasks_and_reminders(self, records: list) -> None:
        ...
        for item in records:
            ...
            if "提醒我" in text:
                ...
                self.personal_state.create_task(text[:300], source_conversation=item.message_id)
