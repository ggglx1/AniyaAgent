...
class QaService:
    ...
    def ask(self, question: str, topic_id: str, *, context_limit: int = 8, token=None, built_context=None, image_blocks=None, run_context=None) -> str:
        track_id = f"qa:{topic_id}"
        ...
        prefix = getattr(built_context, "text", "") or ""
        content = f"{prefix}\n\n{question}" if prefix else question
        if image_blocks:
            content = [{"type": "text", "text": content}, *image_blocks]
        messages.append({"role": "user", "content": content})
        if token is not None: token.check()
        response = self.llm_gateway.messages.create(task_type="main", model=self.model, max_tokens=1024, system="Answer the knowledge question directly and concisely. Do not use tools or personal memory.", messages=messages, tools=[], cancellation_token=token, run_context=run_context)
        if token is not None: token.check()
