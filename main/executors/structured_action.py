...
from main.actions import ActionRegistry, StructuredCommand, StructuredCommandParser
...
class StructuredActionExecutor:
    ...
    def execute(self, request, context, decision):
        ...
        if spec and spec.requires_confirmation and not pending_id:
            pending = store.create(request.track_id, command.to_dict(), [], owner_id=request.user_id, confirmation_state="confirmation_required", source_run_id=request.run_id, source_message_id=command.source_message_id, risk_level=spec.risk_level)
            return UnifiedRunResult(request.run_id, "waiting_confirmation", "This operation is irreversible. Reply 'confirm' to continue.", metadata={"executor": "structured_action", "pending_action": pending})
